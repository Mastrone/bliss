...
std::vector<protohit> bliss::protohit_search(bliss::frequency_drift_plane &drift_plane, noise_stats noise_estimate, hit_search_options options) {
    ...
    // 1. Prepare Noise Statistics per Drift Rate
    // Different drift rates involve summing different numbers of bins (due to desmearing).
    // We must adjust the noise threshold accordingly:
    // Noise power scales with sqrt(N_bins_summed).
    ...
    for (auto &drift_rate : drift_plane.drift_rate_info()) {
        // Calculate the noise level expected for this specific drift trajectory
    ...
    // 2. Execute the selected search algorithm
