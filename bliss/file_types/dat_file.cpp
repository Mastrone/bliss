...
void bliss::write_scan_hits_to_dat_file(scan scan_with_hits, std::string_view file_path, double max_drift_rate) {
    ...
    // [RUN 1] Questo deve rimanere, serve per trovare le hits
    ...
    // --- IL FIX È QUI ---
    // PRIMA C'ERA: auto drift_range = scan_with_hits.get_drift_range(); (Lento, Ricalcolava tutto)
    ...
    std::string header =
            fmt::format("# -------------------------- o --------------------------\n"
                        ...
                        max_drift_rate, // <--- Qui usiamo il valore passato, niente più calcoli!
