...
#include "hit.capnp.h" // Generated by capnp compiler
...
void bliss::write_hits_to_capnp_file(Container hits, std::string_view file_path) {
    ...
    for (auto &this_hit : hits) {
        ...
        // Convert internal hit struct to capnp message
...
std::list<hit> bliss::read_hits_from_capnp_file(std::string_view file_path) {
    ...
    // Read loop for multiple messages in one file
    while (true) {
        try {
            ...
            // End of file reached (or error), break loop
            break;
        }
    }
